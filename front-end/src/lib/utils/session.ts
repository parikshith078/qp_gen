...
export function extractTokensFromHeaders(headers: any): TokensResult {
  ...
  if (Array.isArray(cookies)) {
    cookies.forEach(cookie => {
      ...
      const nameValuePair = firstSemicolonIndex !== -1 ? 
                            cookie.substring(0, firstSemicolonIndex) : 
                            cookie;
      ...
      if (firstSemicolonIndex !== -1) {
        const parts = cookie.substring(firstSemicolonIndex + 1).split(';').map(part => part.trim());
